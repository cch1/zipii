...
;;   z/TreeLike
;;   (tree [_] (z/tree (z/seed treeish (map z/tree (concat (reverse lefts) [mtree] rights)))))
;;   (branch? [_] true)
;;   (branches [_] (concat lefts [mtree] rights))
;;   (seed [_ ls mt rs] (Siblings. ls mt rs treeish))
;;   (seed [_ branches] (z/seed treeish branches)))

;; (defmethod print-method Siblings [s w] (let [[l m r] s] (.write w "-<") (.write w (str (vector l m r))) (.write w ">-")))

;; (defn- section->siblings [section item] (assert (instance? com.hapgood.zipii.loc.Section section)) (->Siblings () item (z/branches section) section))

;; (defrecord SLoc [t mpath ptrees ->treeish]
;;   z/TreeLike
;;   (tree [this] (z/tree t))
;;   #_ (children [this] (when (z/branch? tree) (children tree)))
;;   z/Zipper
;;   (left [this] (when (not= top mpath)
;;                  (let [[[l & ls :as lefts] p' rights] mpath
;;                        node [(or ls ()) p' (cons t rights)]]
;;                    (when l (->SLoc l node ptrees ->treeish)))))
;;   (right [this] (when (not= top mpath)
;;                   (let [[lefts p' [r & rs :as rights]] mpath
;;                         node [(cons t lefts) p' (or rs ())]]
;;                     (when r (->SLoc r node ptrees ->treeish)))))
;;   (up [this] (when (not= top mpath)
;;                (let [[lefts p' rights] mpath]
;;                  (->SLoc (z/seed (peek ptrees) lefts t rights) p' (pop ptrees) ->treeish))))
;;   (down [this] (when-let [branches (z/branches t)]
;;                  (when (seq branches)
;;                    (let [[lmts mt rmts :as s] t]
;;                      (->SLoc (->treeish mt) [lmts mpath rmts] (conj ptrees t) ->treeish)))))
;;   (change [this t] (->SLoc (->treeish t) mpath ptrees ->treeish))
;;   (insert-left [this l] (if (not= top mpath)
;;                           (let [[lefts p rights] mpath
;;                                 node [(cons (->treeish l) lefts) p rights]]
;;                             (->SLoc t node ptrees ->treeish))
;;                           (throw (ex-info "Can't insert left of top" {:loc this :tree t}))))
;;   (insert-right [this r] (if (not= top mpath)
;;                            (let [[lefts p rights] mpath
;;                                  node [lefts p (cons (->treeish r) rights)]]
;;                              (->SLoc t node ptrees ->treeish))
;;                            (throw (ex-info "Can't insert right of top" {:loc this :tree t}))))
;;   (insert-down [this t1] (if-let [branches (z/branches t)] ; branch?
...
;;   (delete [this] (if (not= top mpath)
;;                    (let [[[l & ls :as lefts] parent [r & rs :as rights]] mpath]
;;                      (cond
;;                        r (->SLoc r [lefts parent (or rs ())] ptrees ->treeish)
;;                        l (->SLoc l [(or ls ()) parent rights] ptrees ->treeish)
;;                        true (->SLoc (z/seed (peek ptrees) ()) parent (pop
;;                                                                       ptrees) ->treeish)))
;;                    (throw (ex-info "Can't remove at top" {:loc this :tree t})))))


;; ;; TODO: unify branch? and branches
;; (defn- make->streeish [branch?* branches* seed*] (comp (fn [t] (if-let [[c & cs] (when (z/branch? t) (seq (z/branches t)))]
;;                                                                  (->Siblings () c (or cs ()) t)
;;                                                                  t))
...
;;   (let [->treeish (make->streeish branch? children section)]
